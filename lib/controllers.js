...
exports.surbitcoin = function($){
  client.get('https://api.blinktrade.com/api/v1/VEF/ticker', function(data, response){
      ...
      var rate_vef = parseFloat(data.buy) + parseFloat(data.sell);
          rate_vef = rate_vef / 2;
...
exports.bitfinex = function($){
  client.get('https://api.bitfinex.com/v1/pubticker/btcusd', function(data, response){
    var rate_usd = parseFloat(data.bid) + parseFloat(data.ask);
        rate_usd = rate_usd / 2;
...
exports.okcoin = function($){
  client.get('https://www.okcoin.com/api/v1/ticker.do?symbol=btc_usd', function(data, response){
    data = JSON.parse(data);
    var rate_usd = parseFloat(data.ticker.buy) + parseFloat(data.ticker.sell);
        rate_usd = rate_usd / 2;
    var bid = parseFloat(data.ticker.buy);
    var ask = parseFloat(data.ticker.sell);
    botan.track($.message, 'Bitfinex');
    $.sendMessage("Estadisticas:  \nCompra " + bid.toFixed(2) + "$ \nVenta " + ask.toFixed(2) + "$ \nPromedio " + rate_usd.toFixed(2) + "$ \n@UnF3rn4nd1t0");
  });
}

exports.futuros = function($){
  client.get('https://www.okcoin.com/api/v1/future_ticker.do?symbol=btc_usd&contract_type=this_week', function(data, response){
    data = JSON.parse(data);
    var rate_usd = parseFloat(data.ticker.buy) + parseFloat(data.ticker.sell);
        rate_usd = rate_usd / 2;
    var bid = parseFloat(data.ticker.buy);
...
exports.bitstamp = function($){
  client.get('https://www.bitstamp.net/api/ticker/', function(data, response){
    var rate_usd = (parseFloat(data.bid) + parseFloat(data.ask)) / 2;
    var bid = parseFloat(data.bid);
    var ask = parseFloat(data.ask);
...
exports.foxbit = function($){
  client.get('https://api.blinktrade.com/api/v1/BRL/ticker', function(data, response){
      data = JSON.parse(data);
      var rate_brl = parseFloat(data.buy) + parseFloat(data.sell);
          rate_brl = rate_brl / 2;
...
exports.lisk = function($){
  client.get('https://poloniex.com/public?command=returnTicker', (data, response) => {
    ...
    client.get('https://www.bitstamp.net/api/ticker/', function(data, response){
      ...
      $.sendMessage("El precio de LISK en ico fue 0.0001821238671 BTC/LSK \nUn lisk cuesta: \nBTC: " + result_lsk + " Ƀ \nUSD: " + result_convert.toFixed(2) + " $ \nGracias por usar el bot \n@UnF3rn4nd1t0");
    });
  });
}
...
exports.dolartoday = function($){
  ...
  $.sendMessage("El autor del bot no se hace responsable del contenido enviado, esto es solo una referencia y se insta a no usarla.");
};
...
exports.ceroMaricoteo = function($){
  $.sendPhotoFromUrl("https://www.anony.ws/i/2016/07/14/12923191_553925881447555_3071425386685494908_n1.jpg");
}

exports.siacoin = function($){
  client.get('https://poloniex.com/public?command=returnTicker', (data, response) => {
    var result_sc = parseFloat(data.BTC_SC.last);

    client.get('https://www.bitstamp.net/api/ticker/', function(data, response){
      var rate_usd = parseFloat(data.bid) + parseFloat(data.ask);
          rate_usd = rate_usd / 2;
      var result_convert = result_sc * rate_usd;
      botan.track($.message, 'Lisk');
      $.sendMessage("Un siacoin cuesta: \nBTC: " + result_sc + " Ƀ \nUSD: " + result_convert.toFixed(2) + " $ \nGracias por usar el bot \n@UnF3rn4nd1t0");
    });
  });
}

exports.poloniex = function($, moneda){
  client.get('https://poloniex.com/public?command=returnTicker', (data, response) => {
    var busqueda = "BTC_"+moneda.toUpperCase();
    var poloniex = data.BTC_+moneda.toUpperCase();
    ...
    var result_poloniex = parseFloat(poloniex.last);

    client.get('https://www.bitstamp.net/api/ticker/', function(data, response){
      var rate_usd = parseFloat(data.bid) + parseFloat(data.ask);
          rate_usd = rate_usd / 2;
      var result_convert = result_poloniex * rate_usd;
      botan.track($.message, 'poloniex');
      $.sendMessage("Su valor es: \nBTC: " + result_poloniex + " Ƀ \nUSD: " + result_convert.toFixed(2) + " $ \nSu volumen promedio es de "+poloniex.quoteVolume+"\nGracias por usar el bot \n@UnF3rn4nd1t0");
    });
  });
}
